    ...
    internal class ProfilerFrameDataTreeView : TreeView
    {
        ...
        protected override IList<TreeViewItem> BuildRows(TreeViewItem root)
        {
            ...
            else
            {
                ...
                // Use selectedFrameIndex, not lastFrameIndex — user navigates frames without lastFrame changing.
                ...
                if (needsRebuild)
                {
                    // Pool existing rows before clearing
        ...
        void CellGUI(Rect cellRect, FrameDataTreeViewItem item, bool needsIndent, int column, ref RowGUIArgs args)
        {
            ...
            var content = new GUIContent(item.columnStrings[column], string.Empty);
            DefaultStyles.label.Draw(cellRect, content, false, false, args.selected, args.focused);
        }
    }

    internal class ProfilerFrameDataMultiColumnHeader : MultiColumnHeader
    {
